...
function restoreData(){
  ...
  input.onchange=()=>{const f=input.files&&input.files[0];if(!f)return;const r=new FileReader();
    r.onload=async()=>{try{const x=JSON.parse(r.result);
      if(typeof restoreDailyReportPhotoBackup==='function')await restoreDailyReportPhotoBackup(x.dailyReportPhotos,x.construction);
...
function cards(){return data.projects.length?data.projects.map(p=>'<div class="card" onclick="openProject('+p.id+')"><span class="tag">'+esc(p.status)+'</span><h3 style="margin-top:7px">'+esc(p.name)+'</h3><div class="muted">'+esc(p.client)+'</div><div style="margin:14px 0 7px" class="bar"><div class="fill" style="width:'+progress(p)+'%"></div></div><div class="muted">'+projectSchedulePeriodLabel(p)+'　｜　'+progress(p)+'%　⚠ '+(p.issueCount||0)+'　☑ '+p.steps.filter(s=>s.done).length+'/'+p.steps.length+'</div></div>').join(''):'<div class="empty">還沒有工程。<br>按右上角＋建立第一個工程。</div>'}
...
function issues(e){nav(e);main.innerHTML=(typeof checklistTabs==='function'?checklistTabs('issues'):'')+'<div class="section"><b>問題追蹤</b><button onclick="newIssue()">＋ 新增</button></div>'+(data.issues.length?data.issues.map(i=>'<div class="card"><span class="tag">'+esc(i.priority)+'</span> <span class="tag">'+esc(i.status)+'</span><h3 style="margin-top:10px">'+esc(i.title)+'</h3><div class="muted">'+esc(i.project)+' · '+esc(i.date)+'</div><p>'+esc(i.note)+'</p></div>').join(''):'<div class="empty">目前沒有問題紀錄。</div>')}
