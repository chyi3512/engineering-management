...
function projectSiteAppointmentsHTML(project,summary=false){
  return projectSiteVisibleItems((project.siteAppointments||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)),summary).map(item=>'<div class="row" style="'+(item.done?'opacity:.52':'')+'">'+(summary?'<label class="site-check"><input type="checkbox" data-site-appointment-check="'+esc(item.id)+'" '+(item.done?'checked':'')+'></label>':'')+'<div style="flex:1">'+esc(item.date)+'<br><span style="'+(item.done?'text-decoration:line-through':'')+'">'+esc(item.text)+'</span><div class="muted">'+(item.generatedItemId?'已加入 '+esc(item.generatedDate)+' 現場確認':'屆期自動加入現場確認')+'</div></div>'+(!item.generatedItemId?'<button class="light" data-site-action="remove-appointment" data-item-id="'+esc(item.id)+'">刪除</button>':'')+'</div>').join('')||'<p class="muted">尚無預約事項</p>';}
