...
function projectSiteBottomHTML(project){
  ...
  return '<section class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+day.date+'"><div class="section"><b>產生今日工程日報</b></div><div class="card"><p class="muted">直接帶入今天的現場確認、到場工種、預約事項與照片。</p><button data-site-action="generate">產生今日工程日報</button><div data-site-message class="muted" role="status"></div></div></section>';
}
...
function projectDailyRecordsHTML(project,date,kind){
  ...
  return rows.map(record=>'<label class="site-check-row" style="'+(record.done?'opacity:.52':'')+'"><input type="checkbox" data-daily-record-check="'+esc(record.id)+'" data-daily-record-date="'+esc(record.date)+'" '+(record.done?'checked':'')+'><span style="flex:1;'+(record.done?'text-decoration:line-through':'')+'">'+esc(record.text)+(record.due?'<small class="muted" style="display:block">截止 '+esc(fmtDate(record.due))+'</small>':'')+(record.done?'<small class="muted" style="display:block">已完成</small>':'')+'</span></label>').join('')||'<div class="muted">尚無'+(kind==='處理'?'現場處理':'待辦事項')+'</div>';
}
function projectDailyOverviewHTML(project,date){
  const day=projectSiteDay(project,date),today=typeof projectCurrentSteps==='function'?projectCurrentSteps(project,date):[],todayHTML=today.map(step=>'<div class="row"><span style="flex:1">'+esc(step.trade||'未分類')+'｜'+esc(step.name||'')+'</span></div>').join('')||'<div class="muted">今天尚無施工項目</div>',appointments=(project.siteAppointments||[]).filter(item=>String(item.date||'')<=date&&(!item.done||item.date===date));
  return '<section class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(date)+'"><div class="section"><b>今天施工</b></div><div class="card">'+todayHTML+'</div><div class="section"><b>現場確認</b></div><div class="card"><div data-site-checks>'+projectSiteChecksHTML(nextSiteConfirmation(project),false)+'</div><button type="button" class="light" data-site-action="show-next-site-add">＋ 新增確認事項</button><form data-site-next-check-form hidden style="margin-top:10px"><input name="text" placeholder="輸入確認事項" required><button type="submit">新增</button></form></div><div class="section"><b>現場紀錄</b></div><div class="card"><b class="muted">現場處理</b><div data-daily-records="處理">'+projectDailyRecordsHTML(project,date,'處理')+'</div><button type="button" class="light" data-site-action="show-daily-record-add" data-record-kind="處理">＋ 新增現場紀錄</button><form data-daily-record-form hidden data-record-kind="處理"><input name="text" placeholder="輸入現場處理紀錄" required><button type="submit">新增</button></form><div style="margin-top:16px"><b class="muted">待辦事項</b><div data-daily-records="待辦">'+projectDailyRecordsHTML(project,date,'待辦')+'</div><button type="button" class="light" data-site-action="show-daily-record-add" data-record-kind="待辦">＋ 新增待辦事項</button><form data-daily-record-form hidden data-record-kind="待辦"><input name="text" placeholder="輸入待辦事項" required><input name="due" type="date"><button type="submit">新增</button></form></div><div style="margin-top:16px"><b class="muted">待確認／協調</b><div data-site-appointments>'+projectSiteAppointmentsHTML({siteAppointments:appointments},true)+'</div><button type="button" class="light" data-site-action="show-appointment-add">＋ 新增後續事項</button><form data-site-appointment-form hidden><label>對象<select name="category">'+dailyOptions(['業主','廠商','工班','管委會','其他'],'業主')+'</select></label><label>事項<input name="text" required></label><label>日期<input type="date" name="date" value="'+esc(date)+'" required></label><label>狀態<select name="status">'+dailyOptions(COORDINATION_STATUSES,'待確認')+'</select></label><button type="submit">新增</button></form></div></div><div class="section"><b>工程照片</b></div><div class="card"><div data-site-photos>'+projectSitePhotosHTML(day,'',false)+'</div><button type="button" class="light" data-site-action="upload">＋ 新增照片</button><input type="file" accept="image/*" multiple data-site-files hidden></div></section>'+projectSiteBottomHTML(project);
}
function projectWeeklyRecordSummaryHTML(project,date){
  const steps=(typeof projectCurrentSteps==='function'?projectCurrentSteps(project,date):[]),rows=steps.map(step=>{
    ...
    return '<div class="row"><div style="flex:1"><b>'+esc(step.trade||'未分類')+'｜'+esc(step.name||'')+'</b><button class="site-item-text" onclick="openProject('+project.id+',\'overview\')">確認清單　'+done+' / '+checks.length+'　›</button><button class="site-item-text" onclick="openProject('+project.id+',\'overview\')">本案工程照片　'+photos.length+' 張　›</button></div></div>';
...
function projectSiteAppointmentsHTML(project,summary=false){
  return projectSiteVisibleItems((project.siteAppointments||[]).slice().sort((a,b)=>a.date.localeCompare(b.date)),summary).map(item=>'<div class="row" style="'+(item.done?'opacity:.52':'')+'">'+(summary?'<label class="site-check"><input type="checkbox" data-site-appointment-check="'+esc(item.id)+'" '+(item.done?'checked':'')+'></label>':'')+'<div style="flex:1">'+esc(item.date)+'　'+esc(item.category||'業主')+'<br><span style="'+(item.done?'text-decoration:line-through':'')+'">'+esc(item.text)+'</span><div class="muted">'+esc(item.status||'待確認')+'</div></div><button class="light" data-site-action="edit-appointment" data-item-id="'+esc(item.id)+'">編輯</button><button class="light" data-site-action="remove-appointment" data-item-id="'+esc(item.id)+'">刪除</button></div>'+(projectSiteEditingAppointmentId===item.id?'<form data-site-appointment-edit-form data-item-id="'+esc(item.id)+'"><label>分類<select name="category">'+dailyOptions(COORDINATION_CATEGORIES,item.category||'業主')+'</select></label><label>事項<input name="text" value="'+esc(item.text)+'" required></label><label>日期<input type="date" name="date" value="'+esc(item.date)+'" required></label><label>狀態<select name="status">'+dailyOptions(COORDINATION_STATUSES,item.status||'待確認')+'</select></label><button type="submit" class="light">儲存</button><button type="button" class="light" data-site-action="cancel-appointment-edit">取消</button></form>':'')).join('')||'<p class="muted">尚無待確認／協調事項</p>';}
...
function bindProjectSite(project){
  for(const root of main.querySelectorAll('[data-site-project]')){
    ...
    root.addEventListener('submit',event=>{
      ...
      if(event.target.matches('[data-daily-record-form]')){event.preventDefault();const form=event.target,text=form.elements.text.value.trim(),date=root.dataset.siteDate;if(!text)return;projectSiteUpdate(project,()=>{const day=projectSiteDay(project,date);day.records=[...(day.records||[]),{id:dailyReportId(),kind:form.dataset.recordKind,text,due:form.elements.due?.value||'',done:false,createdAt:new Date().toISOString()}];});openProject(project.id,'overview');return;}
...
function openProjectSitePhotoUpload(project,date,files,itemId,root){if(!files.length)return;openModal('<div class="project-site"><h2>儲存照片</h2>'+files.map((file,index)=>'<label>'+esc(file.name)+'<input data-project-photo-description="'+index+'" placeholder="照片說明"></label>').join('')+'<div class="actions"><button class="light" onclick="closeModal()">取消</button><button id="projectPhotoSave">儲存照片</button></div></div>');document.getElementById('projectPhotoSave').onclick=()=>{const descriptions=files.map((file,index)=>document.querySelector('[data-project-photo-description="'+index+'"]').value.trim());closeModal();addProjectSitePhotos(project,date,files,itemId,root,descriptions);};}
