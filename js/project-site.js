...
function bindProjectSite(project){
  for(const root of main.querySelectorAll('[data-site-project]')){
    ...
    if(root.dataset.siteStepIndex!==undefined){loadProjectStepPhotoImages(project,root);const step=project.steps?.[Number(root.dataset.siteStepIndex)];if(step)projectInspectionAccordionUpdate(root,step);}
  }
}
