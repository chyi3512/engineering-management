...
async function projectSiteAction(event){
  ...
  try{
    ...
    if(action==='edit-appointment')openProjectSiteAppointment(project.id,button.dataset.itemId);
...
function openProjectSiteAppointment(projectId,itemId){
  const project=dailyProject(projectId),item=(project?.siteAppointments||[]).find(row=>row.id===itemId);if(!item)return;
  openModal('<div class="project-site"><h2>待確認／協調事項</h2><label>分類<select id="coordinationCategory">'+dailyOptions(COORDINATION_CATEGORIES,item.category||'業主')+'</select></label><label>事項<input id="coordinationText" value="'+esc(item.text)+'" required></label><label>日期<input id="coordinationDate" type="date" value="'+esc(item.date)+'" required></label><label>狀態<select id="coordinationStatus">'+dailyOptions(COORDINATION_STATUSES,item.status||'待確認')+'</select></label><div class="actions"><button class="light" onclick="closeModal()">取消</button><button id="coordinationSave">儲存</button></div><p class="muted" id="coordinationMessage"></p></div>');
  document.getElementById('coordinationSave').onclick=()=>{const text=document.getElementById('coordinationText').value.trim(),date=document.getElementById('coordinationDate').value;if(!text||!isCompleteScheduleDate(date)){document.getElementById('coordinationMessage').textContent='請填寫事項與有效日期。';return;}try{projectSiteUpdate(project,()=>{const current=project.siteAppointments.find(row=>row.id===itemId);Object.assign(current,{text,date,category:document.getElementById('coordinationCategory').value,status:document.getElementById('coordinationStatus').value});});closeModal();refreshProjectSite(project);}catch(error){document.getElementById('coordinationMessage').textContent='尚未儲存：'+error.message;}};
}
