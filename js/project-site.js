...
async function generateProjectSiteReport(projectId,date=projectSiteDate()){
  ...
  const day=projectSiteDay(project,date),steps=project.actualSchedule||project.steps||[],todayWork=(day.trades||[]).map(trade=>({trade,content:steps.filter(step=>step.trade===trade&&(step.start||step.end)<=date&&(step.end||step.start)>=date).map(step=>step.name).join('、')||'未記錄施工內容'})),nextStep=steps.filter(step=>isCompleteScheduleDate(step.start||step.end)&&(step.start||step.end)>date).sort((a,b)=>String(a.start||a.end).localeCompare(String(b.start||b.end)))[0],report=JSON.parse(JSON.stringify({...day,id:dailyReportId(),projectName:project.name,client:project.client||'',createdAt:new Date().toISOString(),todayWork,acceptances:steps.filter(step=>step.acceptanceConfirmed===true).map(step=>({label:step.acceptanceName||step.acceptance?.label||(step.name==='保護'?'管委會驗收':'驗收節點')})),nextConstruction:nextStep?[{date:nextStep.start||nextStep.end,trade:nextStep.trade||'未分類',content:nextStep.name||''}]:[],coordinationItems:(project.siteAppointments||[]).map(item=>({category:item.category||'業主',text:item.text,status:item.status||'待確認'}))}));
