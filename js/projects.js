...
function projectNodePhotosHTML(photos){return photos.length?'<div class="node-photo-grid">'+photos.map(item=>'<div class="node-photo-cell"><button class="light node-photo" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'" data-photo-date="'+esc(item.day.date)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案工程照片"></button><input aria-label="照片說明" data-site-caption="'+esc(item.photo.id)+'" data-photo-date="'+esc(item.day.date)+'" value="'+esc(item.photo.description||'')+'" placeholder="補充說明"><details class="site-row-menu"><summary aria-label="照片操作">⋯</summary><div><button class="light" data-site-action="remove-photo" data-photo-id="'+esc(item.photo.id)+'" data-photo-date="'+esc(item.day.date)+'">刪除照片</button></div></details></div>').join('')+'</div>':'<div class="muted node-empty">尚無本案工程照片</div>';}
