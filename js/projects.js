...
function swdStepsGanttHTML(project){
  ...
  const period=projectSchedulePeriod(project),days=scheduleDays(period.start,period.end),dayWidth=44,width=days*dayWidth,dates=[];
  for(let date=dateObj(period.start);date<=dateObj(period.end);date.setDate(date.getDate()+1))dates.push(new Date(date));
  const heads=dates.map((date,index)=>'<div style="min-width:'+dayWidth+'px;text-align:center;font-size:9px;color:#777">'+(index===0||date.getDate()===1||days<=21?(date.getMonth()+1)+'/'+date.getDate():'')+'</div>').join('');
  ...
  return '<div class="swd-gantt-scroll" style="overflow-x:auto"><div style="min-width:'+(114+width)+'px"><div style="display:grid;grid-template-columns:114px '+width+'px;border-bottom:1px solid #ddd"><div style="position:sticky;left:0;z-index:3;background:#fff;padding:8px 6px;font-size:11px;font-weight:700">工種</div><div style="display:grid;grid-template-columns:repeat('+days+','+dayWidth+'px)">'+heads+'</div></div>'+rows+'</div></div>';
}
