...
function projectProtectionNodeHTML(project,step){const stepIndex=(project.steps||[]).indexOf(step),checks=projectProtectionChecks(step),done=checks.filter(item=>item.checked).length,allDone=done===checks.length,refs=projectProtectionReferencePhotos(step),projectPhotos=typeof projectStepPhotos==='function'?projectStepPhotos(project,stepIndex):[],reference=refs.length?'<div style="display:flex;gap:8px;overflow:hidden">'+refs.map(item=>'<img src="'+esc(item.photo.data||item.photo.url||'')+'" alt="'+esc(item.photo.title||'保護案例')+'" style="width:72px;height:72px;object-fit:cover;border-radius:8px">').join('')+'</div>':'<div class="muted">從資料庫查看保護案例，不在此新增</div>',referenceIndex=refs.length?(refs[0].group.source==='2022'?refs[0].group.ti:refs[0].group.customIndex):0,referenceLink=refs.length?'<button class="light" style="margin-top:8px;padding:7px 9px" onclick="openLibraryTrade(\''+esc(refs[0].group.source)+'\','+referenceIndex+')">查看全部參考 ＞</button>':'',photos=projectPhotos.length?'<div class="site-photos">'+projectPhotos.slice(0,3).map(item=>'<div class="site-photo"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案施工照片"></div>').join('')+'</div>':'<div class="muted">尚無本案施工照片</div>';return '<div class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(projectSiteDate())+'" data-site-step-index="'+stepIndex+'" data-site-step-trade="'+esc(step.trade||'')+'" data-site-step-name="'+esc(step.name||'')+'"><div style="margin-top:10px"><b>保護確認 '+done+' / '+checks.length+'</b>'+checks.map(item=>'<label class="site-check-row" style="margin-top:6px"><input type="checkbox" data-protection-check="'+esc(item.id)+'" '+(item.checked?'checked':'')+'><span style="'+(item.checked?'text-decoration:line-through;opacity:.6':'')+'">'+esc(item.label)+'</span><small class="muted">'+(item.checked?'已確認':'未確認')+'</small></label>').join('')+'<form data-protection-add-form style="display:flex;gap:8px;margin-top:8px"><input name="label" placeholder="新增保護確認項目" required><button type="submit" class="light">＋ 新增保護確認</button></form></div><div style="margin-top:14px"><b>保護參考</b><div class="muted" style="margin:4px 0 8px">從資料庫查看保護案例，不在此新增</div>'+reference+referenceLink+'</div><div style="margin-top:14px"><b>本案施工紀錄　'+projectPhotos.length+' 張</b><div style="margin-top:8px">'+photos+'</div><div class="site-actions" style="margin-top:8px"><button type="button" class="light" data-site-action="upload">＋ 新增本案施工照片</button></div><input type="file" accept="image/*" multiple data-site-files hidden></div><div style="margin-top:14px"><b>管委會驗收</b><div class="muted" style="margin-top:4px">'+(step.acceptanceConfirmed?'已完成'+(step.acceptanceConfirmedAt?'｜'+esc(fmtDate(step.acceptanceConfirmedAt.slice(0,10))):''):(allDone?'可確認驗收':'等待保護確認完成'))+'</div>'+(!step.acceptanceConfirmed?'<button class="light" style="margin-top:8px;padding:7px 9px" '+(allDone?'':'disabled')+' onclick="confirmProtectionAcceptance('+project.id+','+stepIndex+')">確認驗收</button>':'')+'</div></div>';}
