...
function projectSummaryItems(title,steps,project){const current=title==='現在施工';return '<div class="section"><b>'+title+'</b></div><div class="card">'+(steps.length?steps.map(step=>{const stepIndex=(project.steps||[]).indexOf(step),reminder=step.hasAcceptanceNode?'<div class="hint"><b>驗收節點</b><br>'+esc(step.acceptanceReminder||'未設定提醒文字')+'<br><span class="muted">狀態：'+(step.acceptanceConfirmed?'已驗收':'待確認')+(step.acceptanceConfirmedAt?'｜'+esc(fmtDate(step.acceptanceConfirmedAt.slice(0,10))):'')+'</span>'+(!step.acceptanceConfirmed?'<br><button class="light" style="margin-top:8px;padding:7px 9px" onclick="confirmStepAcceptance('+project.id+','+stepIndex+')">確認驗收完成</button>':'')+'</div>':'',notes=(step.notes||[]).length?'<div style="margin-top:10px"><b>注意事項</b>'+(step.notes||[]).map(note=>'<div class="muted" style="margin-top:6px">□ '+esc(note)+'</div>').join('')+'</div>':'';return '<div class="row"><div style="flex:1"><b>'+esc(step.trade||'未分類')+'｜'+esc(step.name)+'</b><div class="muted">'+esc(fmtDate(step.start))+' ～ '+esc(fmtDate(step.end))+'</div>'+ (current?reminder+notes:'')+'</div><span class="tag">'+projectStepProgress(step)+'%</span></div>';}).join(''):'<div class="muted">'+(current?'目前沒有排定施工中的工程。':'尚無後續工程')+'</div>')+'</div>';}
...
function projectOverviewHTML(project){
  ...
  const swd=isSWD2601Project(project),timeline=swd?'':'<div class="section"><b>施工時間軸</b></div><div class="card">'+projectGanttHTML(project,true)+'</div>',weekSummary=swd?projectWeekSummaryGanttHTML(project):'',pending=swd?projectSiteTopHTML(project):projectOverviewConfirmationHTML(project);
