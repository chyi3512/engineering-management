...
function projectWeekSummaryGanttHTML(project,viewDate=projectViewDate()){
  ...
  const dayWidth=34,width=dates.length*dayWidth,columns='repeat(7,'+dayWidth+'px)',heads=dates.map(date=>'<div style="width:'+dayWidth+'px;text-align:center;font-size:10px;color:#777">'+esc(fmtDate(date))+'</div>').join(''),grid=dates.map(()=>'<div style="width:'+dayWidth+'px;border-left:1px solid #f1f1ef"></div>').join('');
  const rows=trades.map(trade=>{const bars=items.filter(step=>(step.trade||'未分類')===trade).map(step=>{const start=step.start<week.start?week.start:step.start,end=step.end>week.end?week.end:step.end,left=(scheduleDays(week.start,start)-1)*dayWidth,length=scheduleDays(start,end)*dayWidth;return '<div title="'+esc(step.name)+' '+esc(fmtDate(step.start))+'–'+esc(fmtDate(step.end))+'" style="position:absolute;left:'+left+'px;width:'+length+'px;top:6px;height:22px;background:#444;border-radius:6px;padding:5px 7px;color:#fff;font-size:10px;font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(step.name)+'</div>';}).join('');return '<div style="display:grid;grid-template-columns:86px '+width+'px;border-bottom:1px solid #f1f1ef;min-height:34px"><div style="padding:10px 6px;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"><b>'+esc(trade)+'</b></div><div style="position:relative;height:34px"><div aria-hidden="true" style="position:absolute;inset:0;display:grid;grid-template-columns:'+columns+';pointer-events:none">'+grid+'</div>'+bars+'</div></div>';}).join('');
  const todayTrades=[...new Set(items.filter(step=>step.start<=viewDate&&step.end>=viewDate).map(step=>step.trade||'未分類'))];
  return '<div class="section"><b>本週施工</b><button class="light" style="padding:7px 9px" onclick="openProject('+project.id+',\'schedule\')">查看完整工程表</button></div><div class="card overview-week-card"><div class="overview-week-brief"><div>本週 '+trades.length+' 個工種 · '+items.length+' 項施工</div><div class="muted">所選日期：'+esc(todayTrades.join('、')||'尚無排定施工')+'</div></div><div class="overview-week-table"><div style="min-width:'+(86+width)+'px"><div style="display:grid;grid-template-columns:86px '+width+'px;border-bottom:1px solid #ececea"><div style="padding:8px 6px;font-size:11px;font-weight:500">工種</div><div style="display:grid;grid-template-columns:'+columns+'">'+heads+'</div></div>'+rows+'</div></div></div>';
}
...
function projectNodePhotosHTML(photos){return photos.length?'<div class="node-photo-grid">'+photos.map(item=>{const photo=item.photo,stamp=photo.takenAt||photo.createdAt||photo.uploadedAt,date=stamp&& !Number.isNaN(new Date(stamp).getTime())?new Date(stamp).toLocaleString('sv-SE',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}):item.day.date;return '<div class="node-photo-cell"><button class="light node-photo" data-site-action="preview" data-photo-id="'+esc(photo.id)+'" data-photo-date="'+esc(item.day.date)+'"><img data-step-photo="'+esc(photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案工程照片"></button><div class="node-photo-info"><div class="node-photo-title">'+esc(photo.stepName||photo.title||'本案工程照片')+'</div><details class="node-photo-footer site-record-detail"><summary aria-label="展開照片操作"><time class="muted">'+esc(date)+'</time></summary><div class="site-record-actions"><button class="light" data-site-action="remove-photo" data-photo-id="'+esc(photo.id)+'" data-photo-date="'+esc(item.day.date)+'">刪除照片</button></div></details><input aria-label="照片說明" data-site-caption="'+esc(photo.id)+'" data-photo-date="'+esc(item.day.date)+'" value="'+esc(photo.description||'')+'" placeholder="補充說明…"></div></div>';}).join('')+'</div>':'<div class="muted node-empty">尚無本案工程照片</div>';}
...
function openWaterproofChecklist(projectId,stepIndex){const project=data.projects.find(item=>item.id===projectId),step=project?.steps?.[stepIndex];if(!step)return;const refs=projectWaterproofReferencePhotos(step),reference=refs.length?'<div style="display:flex;gap:8px">'+refs.map(photo=>'<img src="'+esc(photo.data||photo.url||'')+'" alt="'+esc(photo.title||'基礎防水參考')+'" style="width:72px;height:72px;object-fit:cover;border-radius:8px">').join('')+'</div>':'<div class="muted">目前沒有可預覽的資料庫照片</div>';openModal('<div class="project-site"><h2>加入工程 Checklist</h2><label>選擇工種<select disabled><option>泥作</option></select></label><label>選擇施工節點<select disabled><option>基礎防水</option></select></label><div class="section"><b>確認項目</b></div>'+WATERPROOF_CHECKLIST.map(item=>'<div class="row">□ '+esc(item)+'</div>').join('')+'<div class="section">參考照片</div><div class="muted">泥作資料庫 → 基礎防水</div>'+reference+'<div class="actions" style="margin-top:15px"><button class="light" onclick="closeModal()">取消</button><button onclick="saveWaterproofChecklist('+projectId+','+stepIndex+')">加入</button></div></div>');}
...
function projectOverviewHTML(project){
  ...
  return dateControl+projectSummaryItems('現在施工',projectCurrentSteps(project,viewDate),project)+weekSummary+timeline+projectDailyOverviewHTML(project,viewDate)+projectSummaryItems('接下來施工',projectUpcomingSteps(project,viewDate),project)+pending;
}
...
function openProject(id,tab='overview'){
  ...
  const tabs=[['overview','概要'],['schedule','工程表'],['inspection','施工檢查']];
  const swd=isSWD2601Project(p),content=tab==='schedule'?(swd?swdStepsOverviewHTML(p):'')+projectScheduleTableHTML(p):tab==='inspection'?workflowProjectHTML(p):projectOverviewHTML(p);
  const overviewHeader='<button class="back" onclick="home()">← 返回</button><div class="card overview-project-head"><div><h2>'+esc(p.name)+'</h2><span class="tag">'+esc(p.status)+'</span></div><p class="muted">'+esc(projectSchedulePeriodLabel(p))+'｜進度 '+projectTotalScheduleProgress(p)+'%</p><button class="back" onclick="openProjectData('+id+')">專案資料 ›</button></div>';
  ...
  if(tab==='overview'||!tabs.some(item=>item[0]===tab)){const tabStart=main.innerHTML.indexOf('<div class="buildflow-tabs"');main.innerHTML='<div class="project-overview">'+overviewHeader+main.innerHTML.slice(tabStart)+'</div>';}bindProjectSite(p);if(siteError)projectSiteMessage(main,siteError);
}
