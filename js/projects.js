...
function projectProtectionNodeHTML(project,step){const stepIndex=(project.steps||[]).indexOf(step),checks=projectProtectionChecks(step),done=checks.filter(item=>item.checked).length,allDone=done===checks.length,refs=projectProtectionReferencePhotos(step),projectPhotos=typeof projectStepPhotos==='function'?projectStepPhotos(project,stepIndex):[],acceptanceNote=step.acceptanceNote||step.acceptanceNotes||'',reference=refs.length?'<div style="display:flex;gap:8px;overflow-x:auto">'+refs.map(item=>'<button class="light" style="padding:0;flex:none" data-site-action="reference-preview" data-photo-src="'+esc(item.photo.data||item.photo.url||'')+'" data-photo-title="'+esc(item.photo.title||'保護案例')+'"><img src="'+esc(item.photo.data||item.photo.url||'')+'" alt="'+esc(item.photo.title||'保護案例')+'" style="width:64px;height:64px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'<div class="muted">從資料庫查看保護案例</div>',referenceIndex=refs.length?(refs[0].group.source==='2022'?refs[0].group.ti:refs[0].group.customIndex):0,referenceLink=refs.length?'<button class="back" style="float:right;font-size:12px" onclick="openLibraryTrade(\''+esc(refs[0].group.source)+'\','+referenceIndex+')">查看全部 ＞</button>':'',photos=projectPhotos.length?'<div style="display:flex;gap:8px;overflow-x:auto">'+projectPhotos.slice(0,3).map(item=>'<button class="light" style="padding:0;flex:none" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案施工照片" style="width:64px;height:64px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'',acceptance='<details style="margin-top:8px;padding:9px 10px;background:#f7f7f5;border:1px solid #e8e8e4;border-radius:10px"><summary style="display:flex;justify-content:space-between;cursor:pointer"><b>管委會驗收</b><span class="muted">'+(step.acceptanceConfirmed?'✓ 已完成':'未完成')+'</span></summary><div class="muted" style="margin-top:6px">'+(step.acceptanceConfirmed?(step.acceptanceConfirmedAt?esc(fmtDate(step.acceptanceConfirmedAt.slice(0,10))):'已完成'):(allDone?'可確認驗收':'等待保護確認完成'))+(acceptanceNote?'<br>'+esc(acceptanceNote):'')+'</div>'+(!step.acceptanceConfirmed?'<button class="light" style="margin-top:7px;padding:6px 9px" '+(allDone?'':'disabled')+' onclick="confirmProtectionAcceptance('+project.id+','+stepIndex+')">確認驗收</button>':'')+'</details>';return '<div class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(projectSiteDate())+'" data-site-step-index="'+stepIndex+'" data-site-step-trade="'+esc(step.trade||'')+'" data-site-step-name="'+esc(step.name||'')+'">'+acceptance+'<div style="margin-top:12px"><b>保護確認 '+done+' / '+checks.length+'</b>'+checks.map(item=>'<label class="site-check-row" style="min-height:30px;font-size:13px"><input type="checkbox" data-protection-check="'+esc(item.id)+'" '+(item.checked?'checked':'')+' style="width:24px;height:24px;margin:0 8px 0 0"><span style="'+(item.checked?'color:#999':'')+'">'+esc(item.label)+'</span></label>').join('')+'<button type="button" class="back" style="margin-top:7px;font-size:13px" data-site-action="show-protection-add">＋ 新增保護確認項目</button><form data-protection-add-form hidden style="display:flex;gap:8px;margin-top:7px"><input name="label" placeholder="新增保護確認項目" required><button type="submit" class="light" style="padding:7px 9px">新增</button></form></div><div style="margin-top:12px"><b>保護參考</b>'+referenceLink+'<div style="clear:both;margin-top:7px">'+reference+'</div></div><div style="margin-top:12px"><b>本案施工紀錄　<span class="muted">'+projectPhotos.length+' 張</span></b><div style="margin-top:7px">'+photos+'</div><button type="button" class="light" data-site-action="upload" style="margin-top:7px;padding:7px 9px;border:1px solid #ddd">＋ 新增本案施工照片</button><input type="file" accept="image/*" multiple data-site-files hidden></div></div>';}
...
function projectWaterproofNodeHTML(project,step){const stepIndex=(project.steps||[]).indexOf(step),checks=Array.isArray(step.checklist)?step.checklist:[],done=checks.filter(item=>item.checked).length,refs=projectWaterproofReferencePhotos(step),photos=typeof projectStepPhotos==='function'?projectStepPhotos(project,stepIndex):[],referenceHTML=refs.length?'<div style="display:flex;gap:8px;overflow-x:auto;margin-top:7px">'+refs.slice(0,3).map(photo=>'<button class="light" style="padding:0;flex:none" data-site-action="reference-preview" data-photo-src="'+esc(photo.data||photo.url||'')+'" data-photo-title="'+esc(photo.title||'基礎防水參考')+'"><img src="'+esc(photo.data||photo.url||'')+'" alt="'+esc(photo.title||'基礎防水參考')+'" style="width:64px;height:64px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'',projectHTML=photos.length?'<div style="display:flex;gap:8px;overflow-x:auto;margin-top:7px">'+photos.slice(0,3).map(item=>'<button class="light" style="padding:0;flex:none" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案施工照片" style="width:64px;height:64px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'<div class="muted" style="font-size:12px;margin-top:6px">尚無本案施工照片</div>',checkHTML=checks.map(item=>'<label class="site-check-row" style="min-height:48px;font-size:14px"><input type="checkbox" data-waterproof-check="'+esc(item.id)+'" '+(item.checked?'checked':'')+' style="width:23px;height:23px;margin:0 13px 0 0;accent-color:#444"><span style="'+(item.checked?'color:#555':'')+'">'+esc(item.label)+'</span></label>').join('');return '<div class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(projectSiteDate())+'" data-site-step-index="'+stepIndex+'" data-site-step-trade="泥作" data-site-step-name="全室基礎防水"><div style="margin-top:11px"><div class="section" style="margin:0 0 5px"><b>確認清單</b><span class="muted">'+done+' / '+checks.length+'</span></div><button class="light" style="padding:7px 9px;border:1px solid #d5d5d2;background:#fff" onclick="openWaterproofChecklist('+project.id+','+stepIndex+')">＋ 加入確認清單</button>'+checkHTML+'</div><div style="margin-top:12px;padding:9px 10px;background:#f7f7f5;border-radius:9px"><b style="font-size:13px;font-weight:600">注意事項</b>'+WATERPROOF_NOTES.map(note=>'<div class="muted" style="font-size:12px;line-height:1.45;margin-top:3px">• '+esc(note)+'</div>').join('')+'</div><div style="margin-top:12px"><div class="section" style="margin:0"><b>參考照片</b><span class="muted">›</span></div><div class="muted" style="font-size:12px;margin-top:2px">泥作資料庫 → 基礎防水<br>從資料庫查看施工參考，不在此新增</div>'+referenceHTML+'</div><div style="margin-top:12px"><div class="section" style="margin:0"><b>本案工程照片</b><span class="muted">'+photos.length+' 張</span></div>'+projectHTML+'<button type="button" class="light" data-site-action="upload" style="margin-top:7px;padding:7px 9px;border:1px solid #d5d5d2;background:#fff">＋ 新增照片</button><input type="file" accept="image/*" multiple data-site-files hidden></div></div>';}
...
function projectSummaryItems(title,steps,project){const current=title==='現在施工';return '<div class="section"><b>'+title+'</b></div><div class="card">'+(steps.length?steps.map(step=>{const protection=current&&step.trade==='木作'&&step.name==='保護',waterproof=current&&isSWD2601Project(project)&&step.trade==='泥作'&&step.name==='全室基礎防水',reminder=step.hasAcceptanceNode?projectAcceptanceSummaryHTML(project,step):'',notes=(step.notes||[]).length?'<div style="margin-top:10px"><b>注意事項</b>'+(step.notes||[]).map(note=>'<div class="muted" style="margin-top:6px">□ '+esc(note)+'</div>').join('')+'</div>':'',upcomingReminder=!current&&step.trade==='木作'&&step.name==='放樣'?'<div style="margin-top:10px"><b>施工前提醒</b>'+['確認完成面基準','確認浴室止水墩尺寸','確認隔間定位'].map(note=>'<div class="muted" style="margin-top:6px">- '+note+'</div>').join('')+'</div>':'';return '<div class="row"><div style="flex:1"><b>'+esc(step.trade||'未分類')+'｜'+esc(step.name)+'</b><div class="muted">'+esc(fmtDate(step.start))+' ～ '+esc(fmtDate(step.end))+'</div>'+ (current?(waterproof?projectWaterproofNodeHTML(project,step):protection?projectProtectionNodeHTML(project,step):reminder+notes):upcomingReminder)+'</div>'+(!protection?'<span class="tag">'+projectStepProgress(step)+'%</span>':'')+'</div>';}).join(''):'<div class="muted">'+(current?'目前沒有排定施工中的工程。':'尚無後續工程')+'</div>')+'</div>';}
...
function openProject(id,tab='overview'){
  ...
  const swd=isSWD2601Project(p),content=tab==='schedule'?(swd?swdStepsOverviewHTML(p):'')+projectScheduleTableHTML(p):tab==='inspection'?workflowProjectHTML(p):tab==='weekly'?(swd?'':projectSiteTopHTML(p))+projectSiteBottomHTML(p):projectOverviewHTML(p);
...
function acceptanceReminderHTML(step){return '<div style="margin-top:12px;padding:12px;background:#f7f7f5;border:1px solid #e8e8e4;border-radius:10px"><b>驗收節點提醒</b><label class="workflow-check"><input id="hasAcceptanceNode" type="checkbox" '+(step.hasAcceptanceNode?'checked':'')+'>此工項有驗收節點</label><label>驗收提醒文字</label><input id="acceptanceReminder" value="'+esc(step.acceptanceReminder||'')+'" placeholder="例如：保護完成請大樓確認"></div>';}
...
function saveStepEdit(id,i){
  ...
  const next=document.getElementById('snext')?.value.trim()||'',predecessor=document.getElementById('spredecessor')?.value.trim()||'',hasAcceptanceNode=document.getElementById('hasAcceptanceNode')?.checked||false,acceptanceReminder=document.getElementById('acceptanceReminder')?.value.trim()||'';
  Object.assign(s,{name,trade,start,end,scheduleMode:mode,durationDays:scheduleDays(start,end)||duration,predecessor,next,hasAcceptanceNode,acceptanceReminder});
...
function saveStep(id){const project=data.projects.find(p=>p.id===id),name=document.getElementById('n')?.value.trim(),trade=document.getElementById('t')?.value.trim()||'自訂',start=document.getElementById('as')?.value||'',days=Number(document.getElementById('ad')?.value),end=isCompleteScheduleDate(start)&&Number.isInteger(days)&&days>=1?shiftScheduleDate(start,days-1):'',progressInput=document.getElementById('ap'),progress=Number(progressInput?.value),predecessor=document.getElementById('apredecessor')?.value||'';if(!project||!name)return;if(isSWD2601Project(project)&&(!isCompleteScheduleDate(start)||!isCompleteScheduleDate(end)||!Number.isFinite(progress)||progress<0||progress>100)){alert('請填寫有效開始日期、天數與 0 到 100 的進度。');return;}project.steps.push({name,trade,next:document.getElementById('nx')?.value||'',notes:(document.getElementById('nt')?.value||'').split('\n').filter(Boolean),done:progressInput?progress===100:false,...(progressInput?{start,end,scheduleMode:'duration',durationDays:days,progress:Math.round(progress),predecessor:predecessor.includes('｜')?predecessor.split('｜').pop():predecessor}:{})});save();closeModal();openProject(id,'overview')}
