...
function saveStepEdit(id,i){
  ...
  Object.assign(s,{name,trade,start,end,next:v.startsWith('P:')?(p.steps[Number(v.slice(2))]?.name||''):v.startsWith('S:')?v.slice(2):''},progressInput?{progress:Math.round(progressValue),done:progressValue===100}:{},checkInputs.length?{completionChecks:Object.fromEntries(checkInputs.map(input=>[input.dataset.completionCheck,input.checked]))}:{});
