...
function projectWaterproofNodeHTML(project,step){const stepIndex=(project.steps||[]).indexOf(step),checks=Array.isArray(step.checklist)?step.checklist:[],done=checks.filter(item=>item.checked).length,refs=projectWaterproofReferencePhotos(step),photos=typeof projectStepPhotos==='function'?projectStepPhotos(project,stepIndex):[],referenceHTML=refs.length?'<div style="display:flex;gap:8px;overflow:hidden">'+refs.map(photo=>'<button class="light" style="padding:0" data-site-action="reference-preview" data-photo-src="'+esc(photo.data||photo.url||'')+'" data-photo-title="'+esc(photo.title||'基礎防水參考')+'"><img src="'+esc(photo.data||photo.url||'')+'" alt="'+esc(photo.title||'基礎防水參考')+'" style="width:72px;height:72px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'<div class="muted">從資料庫查看保護案例，不在此新增</div>',projectHTML=photos.length?'<div class="site-photos">'+photos.slice(0,3).map(item=>'<div class="site-photo"><button class="site-photo-preview" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案施工照片"></button></div>').join('')+'</div>':'<div class="muted">尚無本案施工照片</div>',checkHTML=checks.map(item=>'<label class="site-check-row" style="margin-top:6px"><input type="checkbox" data-waterproof-check="'+esc(item.id)+'" '+(item.checked?'checked':'')+'><span style="'+(item.checked?'text-decoration:line-through;opacity:.6':'')+'">'+esc(item.label)+'</span></label>').join('');return '<div class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(projectSiteDate())+'" data-site-step-index="'+stepIndex+'" data-site-step-trade="泥作" data-site-step-name="全室基礎防水"><div style="margin-top:10px"><b>注意事項</b>'+WATERPROOF_NOTES.map(note=>'<div class="muted" style="margin-top:6px">- '+esc(note)+'</div>').join('')+'</div><div style="margin-top:14px"><button class="light" style="width:100%;text-align:left" onclick="openWaterproofChecklist('+project.id+','+stepIndex+')">確認清單 '+done+' / '+checks.length+' ＞</button><button class="light" style="margin-top:8px" onclick="openWaterproofChecklist('+project.id+','+stepIndex+')">＋加入確認清單</button>'+checkHTML+'</div><div style="margin-top:14px"><b>參考照片</b><div class="muted" style="margin:4px 0 8px">泥作資料庫 → 基礎防水</div>'+referenceHTML+'</div><div style="margin-top:14px"><b>本案工程照片　'+photos.length+' 張</b><div style="margin-top:8px">'+projectHTML+'</div><div class="site-actions" style="margin-top:8px"><button type="button" class="light" data-site-action="upload">＋新增照片</button></div><input type="file" accept="image/*" multiple data-site-files hidden></div></div>';}
