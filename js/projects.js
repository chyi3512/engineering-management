...
function saveStepEdit(id,i){
  ...
  const v=document.getElementById('snext').value,progressInput=document.getElementById('sp'),progressValue=Number(progressInput?.value),checkInputs=Array.from(document.querySelectorAll('[data-completion-check]'));
  if(progressInput&&(!Number.isFinite(progressValue)||progressValue<0||progressValue>100)){alert('進度請填 0 到 100。');return;}
  Object.assign(s,{name,trade,start,end,scheduleMode:mode,durationDays:scheduleDays(start,end)||duration,next:v.startsWith('P:')?(p.steps[Number(v.slice(2))]?.name||''):v.startsWith('S:')?v.slice(2):''},progressInput?{progress:Math.round(progressValue),done:progressValue===100}:{},checkInputs.length?{completionChecks:Object.fromEntries(checkInputs.map(input=>[input.dataset.completionCheck,input.checked]))}:{});
