...
function projectUpcomingSteps(project,today=projectTodayISO()){
  const future=projectDatedSteps(project).filter(step=>step.start>today&&!step.done&&Number(step.progress)<100).sort((a,b)=>a.start.localeCompare(b.start));
...
function projectSummaryItems(title,steps,project){const current=title==='現在施工';return '<div class="section"><b>'+title+'</b></div><div class="card">'+(steps.length?steps.map(step=>{const reminder=step.hasAcceptanceNode&&step.acceptanceReminder?'<div class="hint"><b>驗收提醒</b><br>'+esc(step.acceptanceReminder)+'<br><span class="muted">'+(step.acceptanceConfirmed?'已確認':'未確認')+'</span></div>':'',notes=(step.notes||[]).length?'<div style="margin-top:10px"><b>注意事項</b>'+(step.notes||[]).map(note=>'<div class="muted" style="margin-top:6px">□ '+esc(note)+'</div>').join('')+'</div>':'';return '<div class="row"><div style="flex:1"><b>'+esc(step.trade||'未分類')+'｜'+esc(step.name)+'</b><div class="muted">'+esc(fmtDate(step.start))+' ～ '+esc(fmtDate(step.end))+'</div>'+ (current?reminder+notes:'')+'</div><span class="tag">'+projectStepProgress(step)+'%</span></div>';}).join(''):'<div class="muted">'+(current?'目前沒有排定施工中的工程。':'尚無後續工程')+'</div>')+'</div>';}
...
function stepDateFieldsHTML(step){const mode=step.scheduleMode==='end'?'end':'duration',duration=stepDurationDays(step);return '<div id="stepScheduleMode" data-mode="'+mode+'"><div class="schedule-view-tabs"><button type="button" class="light" data-step-mode="duration" aria-pressed="'+(mode==='duration')+'" onclick="setStepScheduleMode(\'duration\')">工期</button><button type="button" class="light" data-step-mode="end" aria-pressed="'+(mode==='end')+'" onclick="setStepScheduleMode(\'end\')">指定結束日期</button></div><div style="padding:12px;background:#f7f7f5;border:1px solid #e8e8e4;border-radius:10px"><div class="grid"><div><label>開始日期</label><input id="ss" type="date" value="'+esc(step.start||'')+'" oninput="syncStepDurationEnd()"></div><div id="stepDurationField" '+(mode==='duration'?'':'hidden')+'><label>工期（日）</label><input id="sd" type="number" min="1" step="1" value="'+duration+'" oninput="syncStepDurationEnd()"></div><div id="stepEndField" '+(mode==='end'?'':'hidden')+'><label>結束日期</label><input id="se" type="date" value="'+esc(step.end||'')+'"></div></div><div id="stepComputedEnd" class="hint" '+(mode==='duration'?'':'hidden')+'></div></div></div>';}
...
function saveStepEdit(id,i){
  ...
  const startInput=document.getElementById('ss'),endInput=document.getElementById('se'),mode=document.getElementById('stepScheduleMode')?.dataset.mode||'end',durationInput=document.getElementById('sd'),duration=Number(durationInput?.value),start=startInput.value,end=mode==='duration'&&isCompleteScheduleDate(start)&&Number.isInteger(duration)&&duration>=1?shiftScheduleDate(start,duration-1):endInput.value;
  if(!name)return;
  if(startInput.validity.badInput||endInput.validity.badInput||(mode==='duration'&&(!Number.isInteger(duration)||duration<1))||(start&&!isCompleteScheduleDate(start))||(end&&!isCompleteScheduleDate(end)))return;
