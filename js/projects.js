...
function projectWaterproofNodeHTML(project,step){const stepIndex=(project.steps||[]).indexOf(step),checks=Array.isArray(step.checklist)?step.checklist:[],done=checks.filter(item=>item.checked).length,refs=projectWaterproofReferencePhotos(step),photos=typeof projectStepPhotos==='function'?projectStepPhotos(project,stepIndex):[],referenceHTML=refs.length?'<div style="display:flex;gap:8px;overflow-x:auto;margin-top:10px">'+refs.slice(0,3).map(photo=>'<button class="light" style="padding:0;flex:none" data-site-action="reference-preview" data-photo-src="'+esc(photo.data||photo.url||'')+'" data-photo-title="'+esc(photo.title||'基礎防水參考')+'"><img src="'+esc(photo.data||photo.url||'')+'" alt="'+esc(photo.title||'基礎防水參考')+'" style="width:64px;height:64px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'',projectHTML=photos.length?'<div style="display:flex;gap:8px;overflow-x:auto;margin-top:10px">'+photos.slice(0,3).map(item=>'<button class="light" style="padding:0;flex:none" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案施工照片" style="width:64px;height:64px;object-fit:cover;border-radius:8px"></button>').join('')+'</div>':'<div class="muted" style="font-size:14px;margin-top:10px">尚無本案施工照片</div>',checkHTML=checks.map(item=>'<label class="site-check-row" style="min-height:50px;font-size:16px;border-bottom:1px solid #ececea"><input type="checkbox" data-waterproof-check="'+esc(item.id)+'" '+(item.checked?'checked':'')+' style="width:21px;height:21px;margin:0 13px 0 0;accent-color:#4b4b48"><span style="'+(item.checked?'color:#555':'')+'">'+esc(item.label)+'</span></label>').join('');return '<div class="project-site" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(projectSiteDate())+'" data-site-step-index="'+stepIndex+'" data-site-step-trade="泥作" data-site-step-name="全室基礎防水"><div style="margin:0 0 22px;padding:14px 15px;background:#f8f8f6;border-radius:13px"><b style="font-size:17px;font-weight:600">注意事項</b>'+WATERPROOF_NOTES.map(note=>'<div class="muted" style="font-size:15px;line-height:1.65;margin-top:2px">• '+esc(note)+'</div>').join('')+'</div><div style="margin-top:0"><div style="display:flex;align-items:center;gap:10px;margin-bottom:11px"><b style="font-size:20px;font-weight:600">確認清單</b><span class="muted" style="font-size:16px">'+done+' / '+checks.length+'</span><button class="light" style="margin-left:auto;padding:6px 9px;height:36px;border:1px solid #d5d5d2;background:#fff" onclick="openWaterproofChecklist('+project.id+','+stepIndex+')">＋ 新增項目</button></div>'+checkHTML+'</div><div style="margin-top:25px"><div class="section" style="margin:0"><b style="font-size:19px;font-weight:600">參考照片</b><span class="muted">›</span></div><div class="muted" style="font-size:14px;margin-top:4px">泥作資料庫 → 基礎防水</div>'+referenceHTML+'</div><div style="margin-top:25px"><div class="section" style="margin:0"><b style="font-size:19px;font-weight:600">本案工程照片</b><span class="muted" style="font-size:15px">'+photos.length+' 張</span></div>'+projectHTML+'<button type="button" class="light" data-site-action="upload" style="margin-top:10px;padding:6px 9px;height:36px;border:1px solid #d5d5d2;background:#fff">＋ 新增照片</button><input type="file" accept="image/*" multiple data-site-files hidden></div></div>';}
