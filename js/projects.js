...
function projectNodePhotosHTML(photos){return photos.length?'<div class="node-photo-strip">'+photos.slice(0,3).map(item=>'<div><button class="light node-photo" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案工程照片"></button><div class="muted">'+esc(item.photo.description||'')+'</div></div>').join('')+'</div>':'<div class="muted node-empty">尚無本案工程照片</div>';}
