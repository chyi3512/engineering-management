...
function completionChecksHTML(step){const checks=step.completionChecks||{};if(!String(step.name||'').includes('保護'))return '';return '<details open style="margin-top:12px;padding:12px;background:#f7f7f5;border:1px solid #e8e8e4;border-radius:10px"><summary style="cursor:pointer;font-weight:700">完成條件・驗收檢查</summary><div style="margin-top:8px">'+PROTECTION_COMPLETION_CHECKS.map(([key,label])=>'<label class="workflow-check"><input type="checkbox" data-completion-check="'+key+'" '+(checks[key]?'checked':'')+'>'+label+'</label>').join('')+'<div class="hint">建議拍攝施工前、中、後照片，作為後續工程進場依據。</div></div></details>';}
...
function setStepScheduleMode(mode){const root=document.getElementById('stepScheduleMode');if(!root)return;root.dataset.mode=mode;root.querySelectorAll('[data-step-mode]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.stepMode===mode)));document.getElementById('stepDurationField').hidden=mode!=='duration';document.getElementById('stepEndField').hidden=mode!=='end';if(mode==='duration')syncStepDurationEnd();}
function stepDetail(id,i){let p=data.projects.find(x=>x.id===id),s=p.steps[i];let opts='<option value="">未設定</option>'+p.steps.map((x,j)=>'<option value="P:'+j+'" '+(x.name===s.next?'selected':'')+'>'+esc(x.trade+'｜'+x.name)+'</option>').join(''),predecessorOpts='<option value="">未設定</option>'+p.steps.filter((x,j)=>j!==i).map(x=>'<option '+(x.name===s.predecessor?'selected':'')+'>'+esc(x.trade+'｜'+x.name)+'</option>').join(''),progressField=isSWD2601Project(p)?'<label>進度（%）<input id="sp" type="number" min="0" max="100" value="'+projectStepProgress(s)+'"></label>':'',deleteButton=isSWD2601Project(p)?'<button class="light danger" onclick="deleteProjectStep('+id+','+i+')">刪除工程</button>':'';openModal('<h2>編輯工項</h2><label>工項名稱</label><input id="sn" value="'+esc(s.name)+'"><label>工種</label><input id="st" value="'+esc(s.trade)+'">'+stepDateFieldsHTML(s)+progressField+completionChecksHTML(s)+nextStepSuggestionsHTML(s)+'<div class="hint">設定日期後，「時間／本週進度」會自動判斷本週工程。</div><label>前置工程</label><select id="spredecessor">'+predecessorOpts+'</select><label>後續工程</label><select id="snext">'+opts+'</select><div class="hint">可以銜接到本工程任何工項，例如「水電→木作」。</div><div class="section"><b>注意事項</b><button class="light" onclick="addNote('+id+','+i+')">＋ 新增</button></div>'+s.notes.map((n,j)=>'<div class="row"><div style="flex:1">□ '+esc(n)+'</div><button class="orderbtn" onclick="editNote('+id+','+i+','+j+')">編輯</button><span class="danger" onclick="delNote('+id+','+i+','+j+')">×</span></div>').join('')+(s.libraryTrade&&s.libraryItem?'<div class="hint">來源：工種庫 → '+esc(s.libraryTrade)+'｜'+esc(s.libraryItem)+'　<button class="light" style="padding:5px 8px;margin-left:4px" onclick="viewProjectLibrarySource(\''+esc(s.libraryTrade)+'\',\''+esc(s.libraryItem)+'\')">查看工種庫資料</button></div>':'')+'<div class="actions" style="margin-top:15px">'+deleteButton+'<button class="light" onclick="closeModal()">取消</button><button onclick="saveStepEdit('+id+','+i+')">儲存修改</button></div>');syncStepDurationEnd()}
...
function saveStepEdit(id,i){
  ...
  const v=document.getElementById('snext').value,predecessor=document.getElementById('spredecessor')?.value||'',progressInput=document.getElementById('sp'),progressValue=Number(progressInput?.value),checkInputs=Array.from(document.querySelectorAll('[data-completion-check]'));
  if(progressInput&&(!Number.isFinite(progressValue)||progressValue<0||progressValue>100)){alert('進度請填 0 到 100。');return;}
  Object.assign(s,{name,trade,start,end,scheduleMode:mode,durationDays:scheduleDays(start,end)||duration,predecessor:predecessor.includes('｜')?predecessor.split('｜').pop():predecessor,next:v.startsWith('P:')?(p.steps[Number(v.slice(2))]?.name||''):v.startsWith('S:')?v.slice(2):''},progressInput?{progress:Math.round(progressValue),done:progressValue===100}:{},checkInputs.length?{completionChecks:Object.fromEntries(checkInputs.map(input=>[input.dataset.completionCheck,input.checked]))}:{});
