...
function projectOverviewHTML(project){
  ...
  return '<div class="card buildflow-overview-stats"><div><span class="muted">工期</span><b>'+esc(projectSchedulePeriodLabel(project))+'</b></div><div><span class="muted">總進度</span><b>'+projectTotalScheduleProgress(project)+'%</b></div></div>'+dateControl+projectSummaryItems('現在施工',projectCurrentSteps(project,viewDate),project)+weekSummary+timeline+projectDailyOverviewHTML(project,viewDate)+projectSummaryItems('接下來施工',projectUpcomingSteps(project,viewDate),project)+pending;
}
