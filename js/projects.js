...
function openProject(id){let p=data.projects.find(x=>x.id===id),cur=p.steps.findIndex(x=>!x.done);if(cur<0)cur=p.steps.length-1;let s=p.steps[cur]||{name:'尚無工項',trade:'',notes:[],next:''};main.innerHTML='<button class="back" onclick="home()">← 返回</button><div class="card"><span class="tag">'+esc(p.status)+'</span><h2>'+esc(p.name)+'</h2><div class="muted">'+esc(p.client)+'</div><div style="margin:15px 0 7px" class="bar"><div class="fill" style="width:'+progress(p)+'%"></div></div><b>'+progress(p)+'%</b></div><div class="section"><b>工程進度表</b></div><div class="grid"><button class="light" onclick="openProjectSchedule('+id+',\'baseline\')">預估進度（唯讀）</button><button class="light" onclick="openProjectSchedule('+id+',\'actual\')">現場實際進度</button></div><div class="section"><b>工程檢視</b></div><div class="grid"><button onclick="openProjectView('+id+',\'time\')">◷ 時間／本週進度</button><button onclick="openProjectView('+id+',\'trade\')">☷ 依工種查看</button></div><div class="section"><b>現場實際施工流程</b><div><button class="light" onclick="reorderProject('+id+')">編排</button> <button onclick="addProjectStep('+id+')">＋ 自訂工項</button></div></div><div class="card">'+p.steps.map((x,i)=>'<div class="row" onclick="stepDetail('+id+','+i+')"><div class="dot '+(x.done?'checked':'')+'">'+(x.done?'✓':'')+'</div><div style="flex:1"><b style="'+(x.done?'text-decoration:line-through;color:#999':'')+'">'+esc(x.name)+'</b><div class="muted">'+esc(x.trade)+'　'+(x.start?esc(fmtDate(x.start)+' ～ '+fmtDate(x.end||x.start)):'尚未排定')+'　→ '+esc(x.next||'未設定')+'</div></div></div>').join('')+'</div><div class="section"><b>現在建議處理</b></div><div class="current"><span class="tag">NOW</span><h3 style="margin-top:10px">'+esc(s.name)+'</h3><div class="muted">'+esc(s.trade)+'</div>'+projectConfirmationHTML(id,cur,p.steps[cur])+'<div class="next"><div class="muted">完成後下一步</div><b>→ '+esc(s.next||'請自行設定')+'</b></div><button style="width:100%;margin-top:13px" onclick="completeStep('+id+','+cur+')">完成此工項</button></div><div class="grid"><button onclick="newIssue('+id+')">⚠ 新增問題</button><button onclick="report('+id+')">📝 工程日報</button></div>'}
