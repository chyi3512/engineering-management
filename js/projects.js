...
function swdScheduleGanttHTML(project){
  ...
  const width=Math.max(300,days*dayWidth),trades=swdScheduleTrades(project),ordered=items.slice().sort((a,b)=>trades.indexOf(a.trade)-trades.indexOf(b.trade)||a.start.localeCompare(b.start));
  ...
  const rows=ordered.map(item=>{
    const left=scheduleDays(start,item.start)-1,length=scheduleDays(item.start,item.end);
    return '<div style="display:grid;grid-template-columns:114px '+width+'px;border-bottom:1px solid #eee;min-height:38px"><div style="position:sticky;left:0;z-index:2;background:#fff;padding:8px 6px;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis"><b>'+esc(item.trade)+'</b><br><span style="color:#777">'+esc(item.name)+'</span></div><div style="position:relative;height:38px;background-image:linear-gradient(to right,#eee 1px,transparent 1px);background-size:'+(100/days)+'% 100%"><button title="'+esc(item.name)+' '+esc(fmtDate(item.start))+'–'+esc(fmtDate(item.end))+'" onclick="swdScheduleModal('+project.id+',\''+esc(item.id)+'\')" style="position:absolute;left:'+(left*dayWidth)+'px;width:'+(length*dayWidth)+'px;top:6px;height:25px;background:#444;border-radius:6px;overflow:hidden;padding:5px 7px;color:#fff;font-size:10px;font-weight:700;white-space:nowrap;text-overflow:ellipsis;text-align:left">'+esc(item.name)+'</button></div></div>';
  }).join('');
  return '<div class="swd-gantt-scroll" style="overflow-x:auto"><div style="min-width:'+(114+width)+'px"><div style="display:grid;grid-template-columns:114px '+width+'px;border-bottom:1px solid #ddd"><div style="position:sticky;left:0;z-index:3;background:#fff;padding:8px 6px;font-size:11px;font-weight:700">工種 / 工程名稱</div><div style="display:grid;grid-template-columns:repeat('+days+',1fr)">'+heads+'</div></div>'+rows+'</div></div>';
}
