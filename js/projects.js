...
function projectNodeAcceptance(step,kind){if(step.acceptance)return step.acceptance.enabled?step.acceptance:null;return kind==='protection'?{enabled:true,label:'管委會驗收'}:null;}
...
function projectNodePhotosHTML(photos){return photos.length?'<div class="node-photo-strip">'+photos.slice(0,3).map(item=>'<button class="light node-photo" data-site-action="preview" data-photo-id="'+esc(item.photo.id)+'"><img data-step-photo="'+esc(item.photo.id)+'" data-step-photo-date="'+esc(item.day.date)+'" alt="本案工程照片"></button>').join('')+'</div>':'<div class="muted node-empty">尚無本案工程照片</div>';}
...
function projectNodeTemplateHTML(project,step,options){const stepIndex=(project.steps||[]).indexOf(step),checks=options.checks,done=checks.filter(item=>item.checked).length,photos=typeof projectStepPhotos==='function'?projectStepPhotos(project,stepIndex):[],acceptance=projectNodeAcceptance(step,options.kind),next=projectNodeNextStep(project,step),date=step.start?(fmtDate(step.start)+(step.end&&step.end!==step.start?' ～ '+fmtDate(step.end):'')):'日期未設定',acceptanceHTML=acceptance?'<section class="node-section node-acceptance"><div class="node-section-head"><b>'+esc(acceptance.label||'節點驗收')+'</b><span class="muted">'+(step.acceptanceConfirmed?'已完成':'未完成')+'</span></div><div class="muted">'+(step.acceptanceConfirmed?(step.acceptanceConfirmedAt?'驗收日期：'+esc(fmtDate(step.acceptanceConfirmedAt.slice(0,10))):'已完成'):(done===checks.length?'可確認驗收':'等待確認清單完成'))+'</div>'+(!step.acceptanceConfirmed?'<button class="light node-small-button" '+(done===checks.length?'':'disabled')+' onclick="confirmNodeAcceptance('+project.id+','+stepIndex+')">確認驗收</button>':'')+'</section>':'',notes=(options.notes||[]).filter(Boolean),notesHTML=notes.length?'<section class="node-section node-notes"><div class="node-section-head"><b>注意事項</b></div>'+notes.map(note=>'<div class="muted node-note">• '+esc(note)+'</div>').join('')+'</section>':'',refs=projectNodeReferenceHTML(options.refs,options.referenceSource,options.referenceLink),nextHTML=next?'<div class="node-next-card"><b>'+esc(next.trade||'未分類')+'｜'+esc(next.name)+'</b><div class="muted">'+(next.start?esc(fmtDate(next.start)):'日期未設定')+'</div></div>':'<div class="muted node-empty">尚無後續工序</div>';return '<div class="project-site node-template" data-site-project="'+esc(project.id)+'" data-site-date="'+esc(projectSiteDate())+'" data-site-step-index="'+stepIndex+'" data-site-step-trade="'+esc(step.trade||'')+'" data-site-step-name="'+esc(step.name||'')+'"><section class="node-section node-basic"><div class="node-section-head"><b>基本資訊</b></div><div><b>'+esc(step.trade||'未分類')+'｜'+esc(step.name||'未命名節點')+'</b><div class="muted">'+esc(date)+'</div></div></section>'+acceptanceHTML+notesHTML+'<section class="node-section"><div class="node-section-head"><b>確認清單</b><span class="node-check-count">'+done+' / '+checks.length+'</span><button class="light node-small-button" onclick="addNodeChecklist('+project.id+','+stepIndex+',\''+options.kind+'\')">＋ 新增項目</button></div><div class="node-checklist">'+(checks.map(item=>'<label class="site-check-row node-check-row"><input type="checkbox" data-'+options.kind+'-check="'+esc(item.id)+'" '+(item.checked?'checked':'')+'><span>'+esc(item.label)+'</span></label>').join('')||'<div class="muted node-empty">尚無確認項目</div>')+'</div></section>'+refs+'<section class="node-section"><div class="node-section-head"><b>本案工程照片</b><span class="muted">'+photos.length+' 張</span></div>'+projectNodePhotosHTML(photos)+'<button type="button" class="light node-small-button" data-site-action="upload">＋ 新增照片</button><input type="file" accept="image/*" multiple data-site-files hidden></section><section class="node-section"><div class="node-section-head"><b>接下來施工</b></div>'+nextHTML+'</section></div>';}
