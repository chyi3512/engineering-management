...
function projectWeekSummaryGanttHTML(project){
  const week=projectWeekRange(),items=projectDatedSteps(project).filter(step=>step.start<=week.end&&step.end>=week.start),trades=[...new Set(items.map(step=>step.trade||'未分類'))];if(!items.length)return '<div class="section"><b>本週施工</b><button class="light" style="padding:7px 9px" onclick="openProject('+project.id+',\'schedule\')">查看完整工程表</button></div><div class="card"><div class="muted">本週尚未排定施工。</div></div>';
...
function projectSummaryItems(title,steps,project){const current=title==='現在施工';return '<div class="section"><b>'+title+'</b></div><div class="card">'+(steps.length?steps.map(step=>{const protection=current&&step.trade==='木作'&&step.name==='保護',reminder=step.hasAcceptanceNode?projectAcceptanceSummaryHTML(project,step):'',notes=(step.notes||[]).length?'<div style="margin-top:10px"><b>注意事項</b>'+(step.notes||[]).map(note=>'<div class="muted" style="margin-top:6px">□ '+esc(note)+'</div>').join('')+'</div>':'';return '<div class="row"><div style="flex:1"><b>'+esc(step.trade||'未分類')+'｜'+esc(step.name)+'</b><div class="muted">'+esc(fmtDate(step.start))+' ～ '+esc(fmtDate(step.end))+'</div>'+ (current?(protection?projectProtectionNodeHTML(project,step):reminder+notes):'')+'</div>'+(!protection?'<span class="tag">'+projectStepProgress(step)+'%</span>':'')+'</div>';}).join(''):'<div class="muted">'+(current?'目前沒有排定施工中的工程。':'尚無後續工程')+'</div>')+'</div>';}
...
function projectOverviewHTML(project){
  ...
  const swd=isSWD2601Project(project),timeline=swd?'':'<div class="section"><b>施工時間軸</b></div><div class="card">'+projectGanttHTML(project,true)+'</div>',weekSummary=swd?projectWeekSummaryGanttHTML(project):'',pending=swd?projectSiteTopHTML(project,true):projectOverviewConfirmationHTML(project);
  return '<div class="card buildflow-overview-stats"><div><span class="muted">工期</span><b>'+esc(projectSchedulePeriodLabel(project))+'</b></div><div><span class="muted">總進度</span><b>'+projectTotalScheduleProgress(project)+'%</b></div></div>'+weekSummary+timeline+projectSummaryItems('現在施工',projectCurrentSteps(project),project)+projectSummaryItems('接下來施工',projectUpcomingSteps(project),project)+pending+'<div class="section"><b>施工檢查摘要</b></div><div class="card"><b>'+doneChecks+'/'+checks+' 項已完成</b><div class="muted">既有 Workflow／Checklist 摘要</div></div><div class="section"><b>本週紀錄／日報</b></div><div class="card"><div class="muted">已有 '+(project.siteReports||[]).length+' 份現場日報</div></div>';
}
