...
function isCompleteScheduleDate(s){
  if(!/^\d{4}-\d{2}-\d{2}$/.test(s||''))return false;
  const [y,m,d]=s.split('-').map(Number),date=new Date(s+'T00:00:00');
  return y>=1&&date.getFullYear()===y&&date.getMonth()===m-1&&date.getDate()===d;
}
function dateObj(s){return isCompleteScheduleDate(s)?new Date(s+'T00:00:00'):null}
function scheduleDateRangeValid(start,end){
  return !(isCompleteScheduleDate(start)&&isCompleteScheduleDate(end)&&end<start);
}
...
function renderWeeklyStages(items){
  items=items.map(x=>({...x,trade:scheduleGroupName(x)}));
  ...
  const map={};
  dated.forEach(x=>{
    let d=new Date(weekKey(x.wstart));
    while(d<=x.wend){
      const k=weekKey(d);
      if(!map[k]){
  ...
  return weeks.map(w=>{
    ...
    return '<div class="card" style="padding:18px 16px;margin-bottom:16px">'+
      '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px">'+
        '<div style="min-width:0"><div style="font-size:26px;font-weight:800;line-height:1.15">Week '+weekNo(w.start)+'｜第 '+weekNo(w.start)+' 週</div>'+ 
...
function exampleLibraryItemDetail(ti,j){return renderLibraryItem('2022',ti,j);}
...
function saveScheduleDates(ti,j){
  ...
  if(document.getElementById('dateStart').validity.badInput||document.getElementById('dateEnd').validity.badInput||(start&&!isCompleteScheduleDate(start))||(end&&!isCompleteScheduleDate(end)))return;
...
function editSchedulePlan(ti,j){
  const it=data.trades?.[ti]?.items?.[j];
  if(!Array.isArray(it))return;
  const current=it[5]&&typeof it[5]==='object'?it[5]:{type:'first'};
  const options=scheduleItems().filter(x=>x.ti!==ti||x.j!==j).map(x=>
    '<option value="'+x.ti+':'+x.j+'" '+(current.ti===x.ti&&current.j===x.j?'selected':'')+'>'+esc(x.trade+'｜'+x.name)+'</option>'
  ).join('');
  openModal('<h2>編輯施工安排</h2><label>進場關係</label><select id="schedulePlanType">'+
    '<option value="first" '+(current.type==='first'?'selected':'')+'>第一階段</option>'+
    '<option value="same" '+(current.type==='same'?'selected':'')+'>與指定工程同步</option>'+
    '<option value="after" '+(current.type==='after'?'selected':'')+'>等待指定工程完成</option>'+
    '<option value="later" '+(current.type==='later'?'selected':'')+'>在指定工程之後</option>'+
    '</select><label>參考工程項目</label><select id="schedulePlanTarget"><option value="">未設定</option>'+options+'</select>'+
    '<div class="actions"><button class="light" onclick="closeModal()">取消</button><button onclick="window.saveSchedulePlan('+ti+','+j+')">儲存</button></div>');
}

function saveSchedulePlan(ti,j){
  const it=data.trades?.[ti]?.items?.[j];
  if(!Array.isArray(it))return;
  const type=document.getElementById('schedulePlanType')?.value||'first';
  const target=document.getElementById('schedulePlanTarget')?.value||'';
  if(type==='first'||!target){it[5]={type:'first'};}
  else{
    const [targetTrade,targetItem]=target.split(':').map(Number);
    it[5]=Number.isInteger(targetTrade)&&Number.isInteger(targetItem)
      ? {type,ti:targetTrade,j:targetItem}
      : {type:'first'};
  }
  save();
  closeModal();
...
function getScheduleGroups(){
  ...
  const addGroup=(rawName, meta)=>{
    ...
    seen.add(name);
    // 舊的種子資料使用 order；目前儲存的分類使用 number。
    // 兩者都只代表大分類的顯示順序，不涉及工程項目資料。
    const n=Number(meta?.number ?? meta?.order);
    groups.push({
      name,
      key:name,
...
function saveScheduleGroupEdit(index){
  ...
  // 尚未寫入自訂分類的舊項目，是以原工種名稱作為分類；改名時需將它們
  // 明確寫入新分類，避免重新繪製後又回到舊工種分類。
  (Array.isArray(data.trades)?data.trades:[]).forEach(t=>{
    (Array.isArray(t.items)?t.items:[]).forEach(it=>{
      if(!Array.isArray(it))return;
      const explicit=String(it[9]||'').trim();
      const current=explicit
        ? normalizeScheduleGroupName(explicit)
        : normalizeScheduleGroupName(t?.name||'未分類');
      if(current===old)it[9]=next;
    });
  });
...
function saveNewScheduleItem(groupIndex){
  ...
  if(document.getElementById('newScheduleItemStart')?.validity.badInput||document.getElementById('newScheduleItemEnd')?.validity.badInput||(start&&!isCompleteScheduleDate(start))||(end&&!isCompleteScheduleDate(end)))return;
...
// 進度表標題列的「＋ 新增」：先選擇大分類，再開啟既有的工程細項表單。
function newScheduleItem(){
  const groups=getScheduleGroups();
  if(!groups.length){alert('請先新增工程分類。');return;}
  openModal('<h2>新增工程細項</h2><label>新增到分類</label><select id="newScheduleItemGroup">'+
    groups.map((g,i)=>'<option value="'+i+'">'+esc(g.name)+'</option>').join('')+
    '</select><div class="actions"><button class="light" onclick="closeModal()">取消</button><button onclick="window.openNewScheduleItemForm()">下一步</button></div>');
}

function openNewScheduleItemForm(){
  const index=Number(document.getElementById('newScheduleItemGroup')?.value);
  if(!Number.isInteger(index)||index<0)return;
  closeModal();
  addScheduleItemToGroup(index);
}


function createScheduleGroup(name){
  // 大分類只寫入 scheduleGroups；不建立工程項目或工種容器。
  const raw=String(name||'').trim();
  if(!raw){alert('請輸入分類名稱。');return false;}
  const groups=getScheduleGroups();
  const n=normalizeScheduleGroupName(raw);
  if(groups.some(g=>normalizeScheduleGroupName(g.name)===n)){alert('已有相同分類名稱。');return false;}
...
function addScheduleGroup(){
  // 不使用原生 prompt：在 IFRAME／嵌入式環境中可能被抑制而沒有任何畫面回應。
  openModal(
    '<h2>新增工程分類</h2>'+
    '<label>分類名稱</label>'+
    '<input id="newScheduleGroupName" placeholder="例如：清潔" autocomplete="off">'+
    '<div class="hint">新增的是工程進度表的大分類；尚不會建立工程項目。</div>'+
    '<div class="actions"><button class="light" onclick="closeModal()">取消</button><button onclick="window.saveScheduleGroupFromModal()">新增分類</button></div>'
  );
  setTimeout(()=>document.getElementById('newScheduleGroupName')?.focus(),50);
}

function saveScheduleGroupFromModal(){
  const name=document.getElementById('newScheduleGroupName')?.value||'';
  if(!createScheduleGroup(name))return;
  closeModal();
}
...
function deleteScheduleItem(ti,j){
  const it=data.trades?.[ti]?.items?.[j];
  if(!Array.isArray(it))return;
  if(!confirm('確定刪除「'+(it[0]||'這個工程項目')+'」？'))return;
  data.trades[ti].items.splice(j,1);
  save();
  exampleSchedule();
}
...
function editScheduleItem(ti,j){
  ...
  return editTradeItem(ti,j,true);
}
...
function updateScheduleDateInline(ti,j,field,value,input){
  if(input?.validity?.badInput || (value && !isCompleteScheduleDate(value)))return;
  const it=data.trades?.[ti]?.items?.[j];
  if(!Array.isArray(it)||!['start','end'].includes(field))return;
  if(String(it[field==='start'?3:4]||'')===String(value||''))return;
  const start=field==='start'?(value||''):String(it[3]||'');
  const end=field==='end'?(value||''):String(it[4]||'');
  if(!scheduleDateRangeValid(start,end)){alert('結束日期不能早於開始日期');return;}
  if(field==='start')it[3]=start;
  if(field==='end')it[4]=end;
  save();exampleSchedule();
}
...
function exampleScheduleUnsafe(e){
  nav(e);
  const items=scheduleItems();
  const validItems=items.filter(x=>x && (!x.start || isCompleteScheduleDate(x.start)) && (!x.end || isCompleteScheduleDate(x.end)));
  ...
  const tradeRows=groupNames.map((groupName,idx)=>{
    ...
    const body=list.length ? list.map(x=>{
      ...
      return '<tr>'+
        ...
        '<td style="padding:8px;border-bottom:1px solid #eee;white-space:nowrap"><input type="date" value="'+esc(x.start||'')+'" onclick="event.stopPropagation()" onblur="updateScheduleDateInline('+x.ti+','+x.j+',\'start\',this.value,this)" style="margin:0;padding:8px;width:145px"></td>'+
        '<td style="padding:8px;border-bottom:1px solid #eee;white-space:nowrap"><input type="date" value="'+esc(x.end||'')+'" onclick="event.stopPropagation()" onblur="updateScheduleDateInline('+x.ti+','+x.j+',\'end\',this.value,this)" style="margin:0;padding:8px;width:145px"></td>'+
  ...
  main.innerHTML=exampleScheduleTabs()+
    ...
    '<div data-example-view="timeline"><div class="section"><b>建議施工時間圖</b><span class="muted">與下方日期連動</span></div>'+ 
    '<div class="card" style="padding:10px">'+timeline+'</div></div>'+ 
    '<div data-example-view="week"><div class="section"><b>各階段重點</b><span class="muted">每週</span></div>'+weekly+'</div>'+
    '<div data-example-view="trade">'+renderExampleTradeView(items)+'</div>'+
    '<div class="section"><div><b>工程進度表</b><div class="muted">依自訂分類分組；分類名稱、編號顯示與分類順序都可以自己調整。</div></div><div style="display:flex;gap:6px;flex-wrap:wrap"><button type="button" class="light" data-action="add-schedule-group">＋ 分類</button><button class="light" onclick="cleanupScheduleGroups()">整理分類</button><button class="light" onclick="collapseAllTradeSchedules()">全部收合</button></div></div>'+ 
    (tradeRows||'<div class="empty">目前沒有工程項目。</div>');
  setExampleScheduleView(exampleScheduleView);
}
...
// 工程進度表由 innerHTML 建立；明確掛到 window，供事件委派與 modal 按鈕可靠呼叫。
window.addScheduleGroup=addScheduleGroup;
window.saveScheduleGroupFromModal=saveScheduleGroupFromModal;
window.saveNewScheduleItem=saveNewScheduleItem;
window.newScheduleItem=newScheduleItem;
window.openNewScheduleItemForm=openNewScheduleItemForm;
window.editSchedulePlan=editSchedulePlan;
window.saveSchedulePlan=saveSchedulePlan;

let exampleScheduleView='timeline';
function exampleScheduleTabs(){
  return '<div class="schedule-view-tabs">'+[['timeline','主頁｜施工時間圖'],['week','以週檢視'],['trade','以工種分類']].map(([key,label])=>'<button type="button" class="light" data-example-tab="'+key+'" onclick="setExampleScheduleView(\''+key+'\')">'+label+'</button>').join('')+'</div>';
}
function setExampleScheduleView(view){
  exampleScheduleView=['timeline','week','trade'].includes(view)?view:'timeline';
  document.querySelectorAll('[data-example-view]').forEach(el=>el.hidden=el.dataset.exampleView!==exampleScheduleView);
  document.querySelectorAll('[data-example-tab]').forEach(el=>el.setAttribute('aria-pressed',String(el.dataset.exampleTab===exampleScheduleView)));
}
function renderExampleTradeView(items){
  return getScheduleGroups().map(g=>'<details class="card" open><summary><b>'+esc(g.name)+'</b></summary>'+items.filter(x=>scheduleGroupName(x)===g.name).map(x=>'<div class="row"><div><b>'+esc(x.name)+'</b><div class="muted">'+esc(x.start||'未設定')+' ～ '+esc(x.end||'未設定')+'</div></div></div>').join('')+'</details>').join('');
}
