...
assert.equal((html.match(/class="site-record-group record-section"/g)||[]).length,3,'three peer sections');
assert.equal((html.match(/class="record-section-header"/g)||[]).length,3,'shared headers');
assert.equal((html.match(/class="record-section-add"/g)||[]).length,3,'three lightweight add actions');
assert.ok(!html.includes('site-followup-add')&&!html.includes('⋯'));
...
assert.ok(photoHTML.indexOf('<time')<photoHTML.indexOf('data-site-caption'),'time before supplemental caption');
...
assert.ok(compact.includes('record-menu')&&compact.includes('inline-record')&&compact.includes('⋯'));
...
assert.ok(completed.includes('record-menu-items'));
...
assert.ok(recordHTML.includes('⋯')&&recordHTML.includes('inline-record')&&recordHTML.includes('remove-daily-record'));
assert.ok(recordHTML.indexOf('data-daily-record-check')<recordHTML.indexOf('<details'));
c.projectWeekRange=()=>({start:'2026-09-14',end:'2026-09-20'});
c.projectDatedSteps=()=>[{trade:'泥作',name:'防水',start:'2026-09-17',end:'2026-09-18'}];
c.dateObj=d=>new Date(d+'T00:00:00Z');
c.timelineDateISO=d=>d.toISOString().slice(0,10);
c.scheduleDays=(a,b)=>Math.round((new Date(b)-new Date(a))/86400000)+1;
const weekHTML=c.projectWeekSummaryGanttHTML(project,'2026-09-18');
assert.ok(weekHTML.includes('本週 1 個工種 · 1 項施工')&&weekHTML.includes('所選日期：泥作'));
assert.ok(weekHTML.includes('overview-week-table')&&weekHTML.includes('查看完整工程表'));
