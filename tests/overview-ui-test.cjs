...
assert.equal((html.match(/class="site-record-group"/g)||[]).length,2,'handling and follow-up groups');
assert.ok(html.includes('<h3>後續事項</h3><h4>待辦</h4>'));
assert.equal((html.match(/＋ 新增後續事項/g)||[]).length,1,'one follow-up add entry');
assert.ok(!html.includes('新增待辦事項'));
...
assert.equal((photoHTML.match(/class="node-photo-footer site-record-detail"/g)||[]).length,5,'expandable date footer per photo');
assert.ok(!/[▸▶⋯…]/.test(photoHTML.replaceAll('補充說明…','')));
...
const compact=c.projectSiteAppointmentsHTML({siteAppointments:[{id:'a',date:'2026-09-18',text:'磁磚發包',category:'廠商'}]},true,true);
