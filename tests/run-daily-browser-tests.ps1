param([string[]]$Phases=@('create','edit','verify'),[int]$Port=9226)
...
foreach($phase in $Phases){
  ...
  Send-DailyCdp 'Page.navigate' @{url="http://127.0.0.1:8766/tests/daily-reports-test.html?phase=$phase"}|Out-Null
  $complete=$false
  for($i=0;$i -lt 180;$i++){
  ...
  $state.results | Set-Content -Encoding UTF8 (Join-Path $PSScriptRoot "daily-$phase-results.txt")
  $state.errors | ConvertTo-Json | Set-Content -Encoding UTF8 (Join-Path $PSScriptRoot "daily-$phase-console.json")
  Write-Output "$phase : $($state.title)"
  Write-Output $state.results
  if(-not $complete -or $state.title -eq 'FAIL'){break}
  if($phase -eq 'mobile'){
    Evaluate-Daily 'document.querySelector("#drChecklist input[type=checkbox]").scrollIntoView({block:"center"});true'|Out-Null
    $point=Evaluate-Daily '(()=>{const e=document.querySelector("#drChecklist input[type=checkbox]"),r=e.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2,checked:e.checked};})()'
    ...
    $checked=Evaluate-Daily 'document.querySelector("#drChecklist input[type=checkbox]").checked'
    if($checked -eq $point.checked){throw 'Touch checkbox did not toggle'}
    Add-Content -Encoding UTF8 (Join-Path $PSScriptRoot 'daily-mobile-results.txt') 'PASS physical touch event toggles checklist'
    Evaluate-Daily 'dailyReportEditor.dirty=false;window.scrollTo(0,0);true'|Out-Null
    $shot=Send-DailyCdp 'Page.captureScreenshot' @{format='png';captureBeyondViewport=$false}
    [IO.File]::WriteAllBytes((Join-Path $PSScriptRoot 'daily-mobile.png'),[Convert]::FromBase64String($shot.data))
    Evaluate-Daily 'document.getElementById("drPhotos").scrollIntoView({block:"start"});true'|Out-Null
    $shot=Send-DailyCdp 'Page.captureScreenshot' @{format='png';captureBeyondViewport=$false}
    [IO.File]::WriteAllBytes((Join-Path $PSScriptRoot 'daily-mobile-photos.png'),[Convert]::FromBase64String($shot.data))
  }
}
$socket.Dispose()
